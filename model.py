...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, return_details=False):
        ...
        # Handle causal mask
        if hasattr(self, 'bias') and T <= self.bias.size(-1):  # 使用注册的 bias
            bias = self.bias[:, :, :T, :T]
        else:  # 动态生成 bias
        ...
        if return_details:
            return y, {"q": q, "k": k, "v": v, "attention_weights": att, "attention_output": y}
        return y
...
class Block(nn.Module):
    ...
    def forward(self, x, return_details=False):
        ...
        attn_output, attn_details = self.attn(attn_input, return_details=True)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_details=False):
        device = idx.device
        b, t = idx.size()
        ...
        for block in self.transformer.h:
            if return_details:
                x, details = block(x, return_details=True)
                details_per_layer.append(details)
            else:
                x = block(x)
        ...
        if return_details:
            return logits, loss,{
                "layer_details": details_per_layer,
                "final_logits": logits,
